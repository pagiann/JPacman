...
import com.jpacman.model.edumode.graph.Graph;
import com.jpacman.model.edumode.graph.GraphAlgorithms;
...
public class TSPSolver {
		...
		while (!openList.isEmpty() && !canceled) {
			...
			if (currentNode.path.size() == numberOfUnvisitedPoints + 1) {
				...
				if ((selectedVariation == CLASSIC)) {
					...
					// remove the start point because pacman start position is at the edge of his
					// current tile
					// and with the start point in the path he travels half tile to the right first
					// (in some solution
					// cases).
				...
				graphDrawable = null;
				SearchAlgorithm.solutionPathPointsDrawable = Collections
						.synchronizedList(new ArrayList<Point>(shortestPath));
				// add again the start point to the drawable path, in order to draw the last
				// edge of the path
	...
	private List<Edge> findNeighborPoints(Graph initialGraph, TSPNode currentNode) {
		...
		for (Edge edge : currentGraph.getEdges()) {
			...
			if (!invalidConnection && edge.isConnectedToVertex(currentPoint)
					&& !visitedPoints.contains(edge.getOppositeVertex(currentPoint))) {
	...
	private Edge getEdgeToNearestPoint(Graph graph, Point currentPoint) {
		...
		// edges are sorted by weight in ascending order so the first one is the one we
		// want
	...
	// Returns a "similar" node from the list, if there is one. Similar is any node
	// that has an equal path list, meaning
