		...
		while (!openList.isEmpty()) {
			...
			if (stepByStepExecution) {
				...
				stepwiseExecutionMiddleStage1(startNode,
						"Expanding node " + currentNode.getMazeGridLocation() + " with\nthe lowest F cost value of\n"
								+ fCostText + " and ID:" + currentNode.id
			...
			for (Point neighborTile : neighborTiles) {
				...
				if (findEqualNodeInList(neighborTile, openList) == null) {
					...
					if (stepByStepExecution) {
						...
						stepwiseExecutionMiddleStage4(neighborNode,
								"Adding neighbor/child node\n" + neighborNode.getMazeGridLocation()
