...
public class TSPSolverRenderer extends EducationalModeRenderer
{
    ...
    public static final UIButton toggleVisuals = new UIButton(new Point(GameApplication.X_OFFSET + Maze.WIDTH + 30, GameApplication.Y_OFFSET + 30), false, new Text("Toggle Visuals", Text.DEFAULT_FONT_SMALL), true);
    ...
    private static final Text variationSelection = new Text("Select a TSP variation", Text.DEFAULT_FONT_LARGE, Screen.YELLOW_COLOR);
    ...
    private static final UIButton classicTSP = new UIButton(new Point(Maze.WIDTH, Maze.HEIGHT / 2 + 15), true, classicVariation, true);
    private static final UIButton customTSP = new UIButton(new Point(Maze.WIDTH, (Maze.HEIGHT / 2) + 65), true, customVariation, true);
    ...
    private static final Text selectedAlgorithmText = new Text("=> Custom A* algorithm with\n   custom heuristic function", Text.DEFAULT_FONT_SMALL, 0x66FFFF);
    ...
    public TSPSolverRenderer(TextRenderer textRenderer, Pacman pacman, TSPSolver tspSolver, PillController pillController)
    {
	...
	toggleGrid = new UIButton(new Point(GameApplication.X_OFFSET + Maze.WIDTH + 30, GameApplication.Y_OFFSET + 40), false, new Text("Toggle Grid", Text.DEFAULT_FONT_SMALL), true);
	minPacmanSpeed = new UIButton(new Point(GameApplication.X_OFFSET + Maze.WIDTH + 70, GameApplication.Y_OFFSET + 90), false, new Text("min", Text.DEFAULT_FONT_MEDIUM), true);
	maxPacmanSpeed = new UIButton(new Point(GameApplication.X_OFFSET + Maze.WIDTH + 230, GameApplication.Y_OFFSET + 90), false, new Text("max", Text.DEFAULT_FONT_MEDIUM), true);
	decreasePacmanSpeed = new UIButton(new Point(GameApplication.X_OFFSET + Maze.WIDTH + 145, GameApplication.Y_OFFSET + 90), false, new Text("-", Text.DEFAULT_FONT_MEDIUM), true);
	increasePacmanSpeed = new UIButton(new Point(GameApplication.X_OFFSET + Maze.WIDTH + 185, GameApplication.Y_OFFSET + 90), false, new Text("+", Text.DEFAULT_FONT_MEDIUM), true);
    }

    public void renderVariationTextStrings(Screen screen)
    {
	textRenderer.renderText(screen, variationSelection, Screen.getMiddleAlignStartPositionInArea(Maze.WIDTH, variationSelection.getWidth()), GameApplication.WINDOW_HEIGHT / 3, true);
	...
	for (int i = 0; i < tspVariations.length; i++) {
		...
		textRenderer.renderText(screen, tspVariations[i].getText(), tspVariations[i].getBounds().x, tspVariations[i].getBounds().y, false);
	    }
	}
    }
	...
	if (Game.getActiveSubMode() == Game.SubMode.TSP_MENU) {
	    ...
	    // toggleVisuals.changeUpperLeftCorner(new Point(GameApplication.X_OFFSET + Maze.WIDTH + 30,
	    // GameApplication.Y_OFFSET + 30), false);
	    // } else {
	    toggleVisuals.moveBoundsLocation(new Point(GameApplication.X_OFFSET + Maze.WIDTH + 30, GameApplication.Y_OFFSET + 10), false);
	    ...
	    textRenderer.renderText(screen, pacmanSpeedValueText, Maze.WIDTH + 30 + pacmanSpeedText.getWidth() + 10, 70, true);
		...
		distanceTraveledText.setTextMessage("Distance traveled:\n " + ((pacman.getDistanceMoved() == 0) ? pacman.getDistanceMoved() : pacman.getDistanceMoved() - Maze.HALF_TILE) + " pixels");
