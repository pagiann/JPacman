...
import static com.jpacman.controller.edumode.AIAlgorithmsInteractiveLearningController.*;
...
public class AIAlgorithmsInteractiveLearningRenderer extends AIAlgorithmsLearningRenderer {
	public static final Text nodesVisitingOrderText = new Text("Visiting order of nodes:\n NORTH=>SOUTH=>WEST=>EAST",
			Text.DEFAULT_FONT_SMALL, 0xFF0066);
	...
	public UIButton difficulty = new UIButton(
			new Point(GameApplication.X_OFFSET + Maze.WIDTH + 160, GameApplication.Y_OFFSET + 128), false,
			easyDifficultyText, true);

	public AIAlgorithmsInteractiveLearningRenderer(TextRenderer textRenderer, Pacman pacman,
		...
		toggleGrid = new UIButton(new Point(GameApplication.X_OFFSET + Maze.WIDTH + 30, GameApplication.Y_OFFSET + 225),
				false, new Text("Toggle Grid", Text.DEFAULT_FONT_SMALL), true);
		minNodeExpansionSpeed = new UIButton(
				new Point(GameApplication.X_OFFSET + Maze.WIDTH + 70, GameApplication.Y_OFFSET + 185), false,
				new Text("min", Text.DEFAULT_FONT_MEDIUM), true);
		maxNodeExpansionSpeed = new UIButton(
				new Point(GameApplication.X_OFFSET + Maze.WIDTH + 230, GameApplication.Y_OFFSET + 185), false,
				new Text("max", Text.DEFAULT_FONT_MEDIUM), true);
		decreaseNodeExpansionSpeed = new UIButton(
				new Point(GameApplication.X_OFFSET + Maze.WIDTH + 145, GameApplication.Y_OFFSET + 185), false,
				new Text("-", Text.DEFAULT_FONT_MEDIUM), true);
		increaseNodeExpansionSpeed = new UIButton(
				new Point(GameApplication.X_OFFSET + Maze.WIDTH + 185, GameApplication.Y_OFFSET + 185), false,
				new Text("+", Text.DEFAULT_FONT_MEDIUM), true);
	}
	...
	public void render(Screen screen) {
		...
		textRenderer.renderText(screen, expandedNodesSpeedValueText,
				Maze.WIDTH + 30 + expandedNodesSpeedText.getWidth() + 10, 165, true);
		...
		if (interactiveExecutionAlgorithm == null) {
			if (selectedAlgorithmIndex == SearchAlgorithm.IDS) {
				...
				textRenderer.renderText(screen, depthStepText, Maze.WIDTH + 30,
						yOffset1RightPanel + yOffsetRightPanel + 10, true);
			...
			if (selectedAlgorithmIndex == SearchAlgorithm.IDS) {
				yOffset1 += 45;
				currentIterationText.setTextMessage("Current Iteration: "
						+ ((IterativeDeepeningAlgorithm) interactiveExecutionAlgorithm).getIteration());
				textRenderer.renderText(screen, currentIterationText, Maze.WIDTH + 30,
						yOffset1RightPanel + yOffsetRightPanel - 15, true);
				currentDepthLimitText.setTextMessage("Current Depth Limit: "
						+ ((IterativeDeepeningAlgorithm) interactiveExecutionAlgorithm).getDepthLimit());
				textRenderer.renderText(screen, currentDepthLimitText, Maze.WIDTH + 30,
			...
			nodesInOpenListText
					.setTextMessage("Nodes in Search Frontier: " + interactiveExecutionAlgorithm.getOpenListSize());
