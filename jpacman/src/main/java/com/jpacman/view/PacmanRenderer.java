...
public class PacmanRenderer implements Renderer
{
	...
	if (pacman.getAnimate()) {
		...
		if (Game.getActiveMode() == Game.Mode.EDUCATIONAL && (Game.getActiveSubMode() == Game.SubMode.CUSTOM_SPACE_PATHFINDING || Game.getActiveSubMode() == Game.SubMode.CLASSIC_TSP)) {
		...
		if (pacman.getDied()) {
			...
			for (int i = 0; i < NUMBER_OF_DYING_SPRITES; i++) {
			    if (animationCounter % animationSpeed >= diedBounds[i] && animationCounter % animationSpeed < diedBounds[i + 1]) {
		    ...
		    if (Game.getActiveMode() == Game.Mode.PLAY || Game.getActiveSubMode() == Game.SubMode.AI_ALGORITHMS_STEPWISE_LEARNING) {
			if (direction == Direction.UP)
			    pacman.setSprite(pacman_up_1);
	...
	if (Game.getActiveMode() == Game.Mode.EDUCATIONAL && (Game.getActiveSubMode() == Game.SubMode.CUSTOM_SPACE_PATHFINDING || Game.getActiveSubMode() == Game.SubMode.CLASSIC_TSP)) {
	    screen.renderGameObject(pacman.getSprite(), pacman.getPresicePosition().x, pacman.getPresicePosition().y, offset, offset);
