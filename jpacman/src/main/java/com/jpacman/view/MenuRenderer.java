...
public class MenuRenderer implements Renderer {
    ...
    public static final int GAME_TITLE_XOFFSET = (GameApplication.WINDOW_WIDTH
            - SpriteSheet.NUMBER_OF_SPRITES_IN_A_ROW * Sprite.SPRITE_SIZE) / 2;
    ...
    static {
        for (int i = 0; i < SpriteSheet.NUMBER_OF_SPRITES_IN_A_ROW; i++) {
            ...
            gameTitle[i + SpriteSheet.NUMBER_OF_SPRITES_IN_A_ROW] = new Sprite(SpriteSheet.icons, Sprite.SPRITE_SIZE, i,
                    1);
            gameTitle[i + 2 * SpriteSheet.NUMBER_OF_SPRITES_IN_A_ROW] = new Sprite(SpriteSheet.icons,
                    Sprite.SPRITE_SIZE, i, 2);
        }
    ...
    // main menu items
    public static UIButton playMode = new UIButton(
            new Point(GameApplication.MENU_X_OFFSET, GameApplication.MENU_Y_OFFSET + Y_OFFSET), true, playModeText,
            true);
    public static UIButton educationalMode = new UIButton(
            new Point(GameApplication.MENU_X_OFFSET, GameApplication.MENU_Y_OFFSET + 2 * Y_OFFSET), true,
            educationalModeText, true);
    public static UIButton about = new UIButton(
            new Point(GameApplication.MENU_X_OFFSET, GameApplication.MENU_Y_OFFSET + 3 * Y_OFFSET), true, abouteText,
            true);
    public static UIButton exit = new UIButton(
            new Point(GameApplication.MENU_X_OFFSET, GameApplication.MENU_Y_OFFSET + 5 * Y_OFFSET), true, exitText,
            true);
    // play mode menu items
    public static UIButton startGame = new UIButton(
            new Point(GameApplication.MENU_X_OFFSET, GameApplication.MENU_Y_OFFSET + Y_OFFSET), true, startGameText,
            false);
    public static UIButton highScores = new UIButton(
            new Point(GameApplication.MENU_X_OFFSET, GameApplication.MENU_Y_OFFSET + 2 * Y_OFFSET), true,
            highScoresText, false);
    public static UIButton controls = new UIButton(
            new Point(GameApplication.MENU_X_OFFSET, GameApplication.MENU_Y_OFFSET + 3 * Y_OFFSET), true, controlsText,
            false);
    // educational mode menu items
    public static UIButton pathFinding = new UIButton(
            new Point(GameApplication.MENU_X_OFFSET, GameApplication.MENU_Y_OFFSET + Y_OFFSET), true, pathfindingText,
            false);
    public static UIButton aiAlgorithmsLearning = new UIButton(
            new Point(GameApplication.MENU_X_OFFSET, GameApplication.MENU_Y_OFFSET + 2 * Y_OFFSET), true,
            aiAlgorithmsLearningText, false);
    public static UIButton tsp = new UIButton(
            new Point(GameApplication.MENU_X_OFFSET, GameApplication.MENU_Y_OFFSET + 3 * Y_OFFSET), true, tspText,
            false);
    public static UIButton back = new UIButton(
            new Point(GameApplication.MENU_X_OFFSET, GameApplication.MENU_Y_OFFSET + 5 * Y_OFFSET), true, backText,
            false);
    ...
    public void render(Screen screen) {
        for (int i = 0; i < SpriteSheet.NUMBER_OF_SPRITES_IN_A_ROW; i++) {
            screen.renderSprite(gameTitle[i], GAME_TITLE_XOFFSET + Sprite.SPRITE_SIZE * i, GAME_TITLE_YOFFSET,
                    Screen.YELLOW_COLOR);
            screen.renderSprite(gameTitle[i + SpriteSheet.NUMBER_OF_SPRITES_IN_A_ROW],
                    GAME_TITLE_XOFFSET + Sprite.SPRITE_SIZE * i, GAME_TITLE_YOFFSET + Sprite.SPRITE_SIZE,
                    Screen.YELLOW_COLOR);
            screen.renderSprite(gameTitle[i + 2 * SpriteSheet.NUMBER_OF_SPRITES_IN_A_ROW],
                    GAME_TITLE_XOFFSET + Sprite.SPRITE_SIZE * i, GAME_TITLE_YOFFSET + (2 * Sprite.SPRITE_SIZE),
                    Screen.YELLOW_COLOR);
        }
        textRenderer.renderText(screen, quoteText, Screen.getMiddleAlignStartPositionInWindow(quoteText.getWidth()),
                4 * Sprite.SPRITE_SIZE, false);
        ...
        if (playMode.isClicked()) {
            ...
            textRenderer.renderText(screen, playMode.getText(), playMode.getBounds().x, playMode.getBounds().y - 100,
                    false);
        } else if (educationalMode.isClicked()) {
            educationalMode.getText().setColor(Screen.YELLOW_COLOR);
            textRenderer.renderText(screen, educationalMode.getText(), educationalMode.getBounds().x,
                    playMode.getBounds().y - 100, false);
        }
        ...
        if (highScores.isClicked()) {
            ...
            textRenderer.renderText(screen, highScores.getText(),
                    Screen.getMiddleAlignStartPositionInWindow(highScores.getText().getWidth()),
                    highScores.getBounds().y - 250, false);
            highScores.getText().setTextMessage("High Scores");

            textRenderer.renderText(screen,
                    new Text("    Name        Date        Score", Text.DEFAULT_FONT_MEDIUM, 0xFFFF00),
                    highScores.getBounds().x - 150, highScores.getBounds().y - 190, false);
            ...
            for (int i = 0; i < ScoreController.topHighScores.size(); i++) {
                ...
                Text entryText = new Text(
                        (i + 1) + "." + strXGap1 + entry.getName() + strXGap2 + entry.getDate() + strXGap3 + highScore,
                        Text.DEFAULT_FONT_MEDIUM);
                textRenderer.renderText(screen, entryText, highScores.getBounds().x - 150,
                        highScores.getBounds().y - 180 + (i + 1) * 26, false);
            }
        }
        ...
        if (controls.isClicked()) {
            ...
            textRenderer.renderText(screen, controls.getText(), controls.getBounds().x, playMode.getBounds().y - 100,
                    false);
