...
public class MenuRenderer implements Renderer {
	...
	static {
		for (int i = 0; i < SpriteSheet.NUMBER_OF_SPRITES_IN_A_ROW; i++) {
			...
			gameTitle[i + SpriteSheet.NUMBER_OF_SPRITES_IN_A_ROW] = new Sprite(SpriteSheet.icons,
					Sprite.SPRITE_SIZE, i,
					1);
	...
	public static UIButton playMode = new UIButton(
			new Point(GameApplication.MENU_X_OFFSET, GameApplication.MENU_Y_OFFSET + Y_OFFSET), true,
			playModeText,
			true);
	...
	public static UIButton about = new UIButton(
			new Point(GameApplication.MENU_X_OFFSET, GameApplication.MENU_Y_OFFSET + 3 * Y_OFFSET), true,
			abouteText,
			true);
	public static UIButton exit = new UIButton(
			new Point(GameApplication.MENU_X_OFFSET, GameApplication.MENU_Y_OFFSET + 5 * Y_OFFSET), true,
			exitText,
			true);
	...
	public void render(Screen screen) {
		for (int i = 0; i < SpriteSheet.NUMBER_OF_SPRITES_IN_A_ROW; i++) {
			screen.renderSprite(gameTitle[i], GAME_TITLE_XOFFSET + Sprite.SPRITE_SIZE * i,
					GAME_TITLE_YOFFSET,
					Screen.YELLOW_COLOR);
			screen.renderSprite(gameTitle[i + SpriteSheet.NUMBER_OF_SPRITES_IN_A_ROW],
					GAME_TITLE_XOFFSET + Sprite.SPRITE_SIZE * i,
					GAME_TITLE_YOFFSET + Sprite.SPRITE_SIZE,
					Screen.YELLOW_COLOR);
			screen.renderSprite(gameTitle[i + 2 * SpriteSheet.NUMBER_OF_SPRITES_IN_A_ROW],
					GAME_TITLE_XOFFSET + Sprite.SPRITE_SIZE * i,
					GAME_TITLE_YOFFSET + (2 * Sprite.SPRITE_SIZE),
					Screen.YELLOW_COLOR);
		}
		textRenderer.renderText(screen, quoteText,
				Screen.getMiddleAlignStartPositionInWindow(quoteText.getWidth()),
		...
		if (playMode.isClicked()) {
			...
			textRenderer.renderText(screen, playMode.getText(), playMode.getBounds().x,
					playMode.getBounds().y - 100,
					false);
		...
		if (about.isClicked()) {
			...
			textRenderer.renderText(screen, about.getText(), about.getBounds().x,
					playMode.getBounds().y - 120, false);
		...
		if (highScores.isClicked()) {
			...
			textRenderer.renderText(screen,
					new Text("    Name        Date        Score", Text.DEFAULT_FONT_MEDIUM,
							0xFFFF00),
			...
			for (int i = 0; i < ScoreController.topHighScores.size(); i++) {
				...
				String highScore = df.format(Integer.valueOf(entry.getHighScore()));
				Text entryText = new Text(
						(i + 1) + "." + strXGap1 + entry.getName() + strXGap2 + entry.getDate()
								+ strXGap3 + highScore,
		...
		if (controls.isClicked()) {
			...
			textRenderer.renderText(screen, controls.getText(), controls.getBounds().x,
					playMode.getBounds().y - 100,
					false);
