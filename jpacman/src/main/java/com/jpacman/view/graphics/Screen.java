...
public class Screen {
	...
	public void renderBounds(Rectangle rectangle, int color, int xOffset, int yOffset, int xPadding, int yPadding,
			boolean useOffsets) {
		int xa; // x absolute
		int ya; // y absolute
	...
	public void renderScreenPainterVisuals(boolean renderMazeGrid, boolean renderHoveredMazeTileLocation) {
		BufferedImage bufferedImage = new BufferedImage(GameApplication.WINDOW_WIDTH, GameApplication.WINDOW_HEIGHT,
		...
		} else if ((Game.getActiveSubMode() == SubMode.CLASSIC_TSP || Game.getActiveSubMode() == SubMode.MAZE_TSP)
				&& TSPSolverController.renderVisuals) {
