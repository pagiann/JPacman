...
public class StatusRenderer implements Renderer {
    ...
    public void render(Screen screen) {
        for (int i = 0; i < pacmanLives - 1; i++) {
            screen.renderGameObject(PacmanRenderer.pacman_left_1,
                    position2.x + Sprite.SPRITE_SIZE + i * Sprite.SPRITE_SIZE, position2.y + Sprite.SPRITE_SIZE, size,
                    size);
        }
        screen.renderGameObject(currentFruit, position.x + Maze.WIDTH - Sprite.SPRITE_SIZE,
                position.y + Sprite.SPRITE_SIZE, size, size);
    }
