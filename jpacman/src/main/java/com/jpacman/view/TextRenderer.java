...
public class TextRenderer implements Renderer {
	...
	// private static SpriteSheet fontSpriteSheet1 = new
	// SpriteSheet("/fonts/Untitled.bmp", 512,
	// SPRITE_SHEET_TILE_SIZE_LARGE, SPRITE_SHEET_TILE_SIZE_LARGE);
	private static SpriteSheet fontSpriteSheet1 = new SpriteSheet("/fonts/large.png", 512, SPRITE_SHEET_TILE_SIZE_LARGE,
			SPRITE_SHEET_TILE_SIZE_LARGE);
	private static SpriteSheet fontSpriteSheet2 = new SpriteSheet("/fonts/medium.png", 384,
			SPRITE_SHEET_TILE_SIZE_MEDIUM, SPRITE_SHEET_TILE_SIZE_MEDIUM);
	private static SpriteSheet fontSpriteSheet3 = new SpriteSheet("/fonts/small.png", 256, SPRITE_SHEET_TILE_SIZE_SMALL,
			SPRITE_SHEET_TILE_SIZE_SMALL);
	...
	public TextRenderer(Point textLabelFixedPosition) {
		this.textLabelFixedPosition = new Point(textLabelFixedPosition.x,
				textLabelFixedPosition.y - Sprite.HALF_SPRITE_SIZE);
	}
	...
	public void render(Screen screen) {
		...
		switch (levelReadyPausedGameOverFlag ^ LEVEL__READY__PAUSED__QUIT__GAME_OVER__MASK) {
			case LEVEL_MASK:
				int xOffset = Screen.getMiddleAlignStartPositionInArea(Maze.TEXT_MESSAGE_AREA_WIDTH,
						level.computeWidth(level.getTextMessage() + levelNumber, level.getFont()));
				renderText(screen, level, textLabelFixedPosition.x + xOffset, textLabelFixedPosition.y + yOffset, true);
				renderText(screen, new Text(levelNumber, Text.DEFAULT_FONT_MEDIUM, Screen.YELLOW_COLOR),
						textLabelFixedPosition.x + xOffset + level.getWidth(), textLabelFixedPosition.y + yOffset,
						true);
				break;
			case READY_MASK:
				renderText(screen, ready, textLabelFixedPosition.x + calculateXOffset(ready),
						textLabelFixedPosition.y + yOffset, true);
				break;
			case PAUSED_MASK:
				renderText(screen, paused, textLabelFixedPosition.x + calculateXOffset(paused),
						textLabelFixedPosition.y + yOffset, true);
				break;
			case QUIT_MASK:
				renderText(screen, quit, textLabelFixedPosition.x + calculateXOffset(quit),
						textLabelFixedPosition.y + yOffset, true);
				break;
			case GAME_OVER__MASK:
				renderText(screen, gameOver, textLabelFixedPosition.x + calculateXOffset(gameOver),
						textLabelFixedPosition.y + yOffset, true);
				break;
		}
	}
	...
	public void renderText(Screen screen, Text text, int x, int y, boolean useOffsets) {
		...
		for (int i = 0; i < textString.length(); i++) {
			...
			if (size > Font.MEDIUM_SIZE) {
				if (italic) {
					screen.renderTextCharacter(x + (int) xOffset, y + (int) (yOffset + line * size),
							largeCharacters[index + 128], color, useOffsets);
				} else {
					screen.renderTextCharacter(x + (int) xOffset, y + (int) (yOffset + line * size),
							largeCharacters[index], color, useOffsets);
				}
			} else if (size == Font.MEDIUM_SIZE) {
				if (italic) {
					screen.renderTextCharacter(x + (int) xOffset, y + (int) (yOffset + 1.3 * line * size),
							mediumCharacters[index + 128], color, useOffsets);
				} else {
					screen.renderTextCharacter(x + (int) xOffset, y + (int) (yOffset + 1.3 * line * size),
							mediumCharacters[index], color, useOffsets);
				}
			} else {
				if (italic) {
					screen.renderTextCharacter(x + (int) xOffset, y + (int) (yOffset + 1.4 * line * size),
							smallCharacters[index + 128], color, useOffsets);
				} else {
					screen.renderTextCharacter(x + (int) xOffset, y + (int) (yOffset + 1.4 * line * size),
							smallCharacters[index], color, useOffsets);
				}
			}
