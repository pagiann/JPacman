...
public class Game {
	...
	private void createNewPlayModeControllers(boolean firstPlay) {
		...
		ghostsControllers[1] = new GhostController(ghosts[1],
				new InkyRenderer((Inky) ghosts[1], Sprite.HALF_SPRITE_SIZE));
		ghostsControllers[2] = new GhostController(ghosts[2],
				new PinkyRenderer((Pinky) ghosts[2], Sprite.HALF_SPRITE_SIZE));
		...
		ghostsStatusControllers[0] = new BlinkyStatusController((Blinky) ghosts[0], levelController, pacman,
				(Clyde) ghosts[3], pillController);
		...
		pacmanCollisionController = new PacmanCollisionController(pacman, ghosts, ghostsAIControllers, fruitController,
				pillController, levelController, scoreController, bonusPointsController);
		pacmanStatusController = new PacmanStatusController(pacman, ghostsStatusControllers, levelController);

		// create status controller (displays pacman lives and current fruit at the
		// bottom of the screen)
		statusController = new StatusController(new StatusRenderer(new Point(Maze.TILE, Maze.HEIGHT + Maze.HALF_TILE),
				Sprite.SPRITE_SIZE, fruitController.getFruit().getSprite()), pacmanStatusController, fruitController);

		// finally create the game status controller
		gameStatusController = new GameStatusController(levelController, ghostsStatusControllers,
				pacmanStatusController, fruitController, pillController, scoreController, mazeRenderer);
	}
	...
	public void updateWorld(double delta) {
		...
		if (activeMode == Mode.PLAY) {
			...
			if (gameOver) {
				...
				((InkyAIController) ghostsAIControllers[1])
						.setBlinkyCurrentTile(ghostsAIControllers[0].getGhost().getCurrentTile());
			...
			introAnimationController.reset();
			if (activeSubMode == SubMode.PATHFINDING_MENU || activeSubMode == SubMode.CUSTOM_SPACE_PATHFINDING
					|| activeSubMode == SubMode.MAZE_PATHFINDING) {
			...
			} else if (activeSubMode == SubMode.TSP_MENU || activeSubMode == SubMode.CLASSIC_TSP
					|| activeSubMode == SubMode.MAZE_TSP) {
		...
		if (activeMode == Mode.PLAY) {
			...
			if (activeSubMode == SubMode.PATHFINDING_MENU || activeSubMode == SubMode.CUSTOM_SPACE_PATHFINDING
				...
				pathfindingController.renderAll(screen);
					...
					|| activeSubMode == SubMode.AI_ALGORITHMS_STEPWISE_LEARNING
					|| activeSubMode == SubMode.AI_ALGORITHMS_INTERACTIVE_LEARNING) {
