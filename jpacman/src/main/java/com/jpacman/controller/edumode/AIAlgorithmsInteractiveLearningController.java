...
import com.jpacman.view.edumode.AIAlgorithmsInteractiveLearningRenderer;
...
public class AIAlgorithmsInteractiveLearningController extends AIAlgorithmsLearningController implements Runnable {
	...
	public AIAlgorithmsInteractiveLearningController(TextRenderer textRenderer) {
		super(textRenderer);
		AIAlgorithmsInteractiveLearningRenderer.selectedAlgorithmText.setTextMessage(
				"=> " + AIAlgorithmsInteractiveLearningRenderer.algorithms[0].getText().getTextMessage());
		...
		aiAlgorithmsInteractiveLearningRenderer = new AIAlgorithmsInteractiveLearningRenderer(textRenderer, pacman,
				searchAlgorithms);
	...
	private void checkIfButtonIsClicked() {
		...
		if (interactiveExecutionAlgorithm == null && !pacman.isMoving()
				&& MenuController.isButtonClicked(aiAlgorithmsInteractiveLearningRenderer.difficulty)) {
			if (aiAlgorithmsInteractiveLearningRenderer.difficulty
					.getText() == AIAlgorithmsInteractiveLearningRenderer.easyDifficultyText) {
				aiAlgorithmsInteractiveLearningRenderer.difficulty
						.setText(AIAlgorithmsInteractiveLearningRenderer.mediumDifficultyText);
				mediumDifficulty = true;
				hardDifficulty = false;
			} else if (aiAlgorithmsInteractiveLearningRenderer.difficulty
					.getText() == AIAlgorithmsInteractiveLearningRenderer.mediumDifficultyText) {
				aiAlgorithmsInteractiveLearningRenderer.difficulty
						.setText(AIAlgorithmsInteractiveLearningRenderer.hardDifficultyText);
				mediumDifficulty = false;
				hardDifficulty = true;
			} else {
				aiAlgorithmsInteractiveLearningRenderer.difficulty
						.setText(AIAlgorithmsInteractiveLearningRenderer.easyDifficultyText);
				mediumDifficulty = false;
				hardDifficulty = false;
			}
		}
	...
	private void checkWhichTileWasClicked() {
		...
		mouseCoordinates.translate(-(GameApplication.X_OFFSET - Maze.HALF_TILE),
				-(GameApplication.Y_OFFSET - Maze.HALF_TILE));
	...
	public void run() {
		...
		switch (algorithmIndex) {
			...
			case SearchAlgorithm.BF:
				currentHeuristicFunction = ((BestFirstAlgorithm) searchAlgorithms[algorithmIndex])
			...
			case SearchAlgorithm.BS:
				currentHeuristicFunction = ((BeamSearchAlgorithm) searchAlgorithms[algorithmIndex])
						.getHeuristicFunction();
