...
public class PathfindingController extends EducationalModeController implements Runnable {
	...
	public PathfindingController(TextRenderer textRenderer) {
		super();
		PathfindingRenderer.selectedAlgorithmText
				.setTextMessage("=> " + PathfindingRenderer.algorithms[0].getText().getTextMessage());
	...
	public void update(double delta) {
		if (selectedVariation == NONE) {
			...
			if (!(searchAlgorithms[algorithmIndex].isRunning() || backCancelButton == cancelButton
					|| pacman.isMoving())) {
			...
			if (!searchAlgorithms[algorithmIndex].isStarted() && Mouse.getButton() == Mouse.RIGHT_BUTTON) {
				...
				if (clickCoordinates.x > Maze.TILE && clickCoordinates.y > Maze.TILE
						&& clickCoordinates.x < Maze.WIDTH - Maze.TILE
						&& clickCoordinates.y < Maze.HEIGHT - Maze.TILE) {
					Point obstacleLocation = new Point(PathFinder.computeCurrentTile(clickCoordinates));
					if (obstacleLocation.equals(PathFinder.computeCurrentTile(pacman.getPosition()))
							|| obstacleLocation.equals(destination)) {
						return;
					}
			...
			if (!searchAlgorithms[algorithmIndex].isStarted() && Mouse.getButton() == Mouse.LEFT_BUTTON) {
				...
				Point clickCoordinates = Mouse.getCoordinates();
				clickCoordinates.translate(-GameApplication.X_OFFSET, -GameApplication.Y_OFFSET);
				if (clickCoordinates.x > 0 && clickCoordinates.y > 0 && clickCoordinates.x < Maze.WIDTH
						&& clickCoordinates.y < Maze.HEIGHT) {
					...
					if (routeTiles.contains(destination) && !start.equals(destination)) {
						...
						if (algorithmIndex != SearchAlgorithm.IDS) {
							...
							pathInNodes = Collections
									.synchronizedList(searchAlgorithms[algorithmIndex].getPathInNodes());
				...
				if (pacman.isMoving()) {
					...
					if (nextExpandedNodeTimer
							.getTimeElapsedInMilliseconds() == nodeExpansionSpeeds[nodeExpansionSpeedIndex]) {
	...
	private void checkWhichButtonIsClicked() {
		...
		if (selectedVariation == CUSTOM_SPACE) {
			...
			if (!searchAlgorithms[algorithmIndex].isStarted()
					&& MenuController.isButtonClicked(pathfindingRenderer.useDiagonalTiles)) {
	...
	private void movePacmanToNextTile() {
		...
		if (selectedVariation == CUSTOM_SPACE) {
			Point pacmanCurrentPresicePosition = new Point((int) Math.rint(pacman.getPresicePosition().x),
			...
			if (pacmanCurrentPresicePosition.equals(pacman.getNextTile())) {
				...
				if (pathInNodes.size() > 0) {
					...
					nextPoint = new Point2D.Float(pathInNodes.get(0).getCoordinates().x,
							pathInNodes.get(0).getCoordinates().y);
				}
			...
			double newAngle = Math.atan2(nextPoint.y - pacman.getPresicePosition().y,
					nextPoint.x - pacman.getPresicePosition().x);
			pacman.getPresicePosition().x += Math.cos(newAngle);
			pacman.getPresicePosition().y += Math.sin(newAngle);
			pacman.setPosition(new Point((int) Math.rint(pacman.getPresicePosition().x),
					(int) Math.rint(pacman.getPresicePosition().y)));
		} else {
	...
	private void addObstacle(Point obstacleLocation) {
		if (SearchAlgorithm.useDiagonalTiles) {
			obstaclesRenderers.add(new TileRenderer(TileRenderer.diagonalMovementObstacles[currentObstacleSprite % 6],
					obstacleLocation, Sprite.HALF_SPRITE_SIZE, Sprite.HALF_SPRITE_SIZE));
			if (++currentObstacleSprite == 6) {
				currentObstacleSprite = 0;
			}
		} else {
			obstaclesRenderers.add(new TileRenderer(TileRenderer.nonDiagonalMovementObstacle, obstacleLocation,
					Sprite.HALF_SPRITE_SIZE, Sprite.HALF_SPRITE_SIZE));
		}
		...
		if (SearchAlgorithm.startNodeDrawable != null
				&& obstacleLocation.equals(SearchAlgorithm.startNodeDrawable.getCoordinates())) {
			SearchAlgorithm.goalNodeDrawable = null;
	...
	private void setNonDiagonalMovementObstacles() {
		...
		while (it.hasNext()) {
			...
			it.remove();
			nonDiagonalMovementObstaclesRenderers.add(new TileRenderer(TileRenderer.nonDiagonalMovementObstacle,
					obstacleLocation, Sprite.HALF_SPRITE_SIZE, Sprite.HALF_SPRITE_SIZE));
		}
	...
	private void setDiagonalMovementObstacles() {
		...
		while (it.hasNext()) {
			Point obstacleLocation = it.next().getLocation();
			it.remove();
			diagonalMovementObstaclesRenderers
					.add(new TileRenderer(TileRenderer.diagonalMovementObstacles[currentObstacleSprite % 6],
							obstacleLocation, Sprite.HALF_SPRITE_SIZE, Sprite.HALF_SPRITE_SIZE));
			if (++currentObstacleSprite == 6) {
				currentObstacleSprite = 0;
			}
