...
public class GraphicalSearchTreeController extends JFrame implements ChangeListener, MouseWheelListener {
	...
	private class DrawCanvas extends JPanel {
		...
		private void paintNodeForAIAlgorithmsLearningMode(Graphics2D g2d, Node node) {
			...
			// paint current node
			if (node.equals(SearchAlgorithm.currentNodeDrawable)
					&& node.getID().equals(SearchAlgorithm.currentNodeDrawable.getID())
					&& !SearchAlgorithm.currentNodeDrawable.equals(SearchAlgorithm.goalNodeDrawable)) {
			...
			if (SearchAlgorithm.currentNeighborNodeDrawable != null
					&& node.getCoordinates().equals(SearchAlgorithm.currentNeighborNodeDrawable.getCoordinates())) {
		...
		private void paintNodeLocationAndHeuristicFunctions(Graphics2D g2d, Node node) {
			...
			if (showNodeLocation) {
				...
				int yDrawLocation;
				if (heuristicFunctionCheckBox.isSelected()) {
					if (EducationalModeController.algorithmIndex == SearchAlgorithm.BS
			...
			if (showHeuristicFunction) {
				...
				if (EducationalModeController.algorithmIndex == SearchAlgorithm.BS
						|| EducationalModeController.algorithmIndex == SearchAlgorithm.A_STAR) {
					g2d.drawString(fCostText, xDrawLocation,
							node.getDrawCoordinates().y - 3 * gapBetweenNodeAndStringLabel);
					g2d.setPaint(G_COST_COLOR);
					g2d.drawString(gCostText, xDrawLocation,
							node.getDrawCoordinates().y - 2 * gapBetweenNodeAndStringLabel + 1);
					g2d.setPaint(H_COST_COLOR);
					g2d.drawString(hCostText, xDrawLocation,
							node.getDrawCoordinates().y - gapBetweenNodeAndStringLabel + 3);
				} else {
					g2d.drawString(hCostText, xDrawLocation2,
							node.getDrawCoordinates().y - gapBetweenNodeAndStringLabel + 2);
				}
			}
		}
		...
		private void scrollToCurrentNode(Node node) {
			if (autoscrolling) {
				Point viewableCurrentNodeDrawCoordinatesScaled = new Point((int) (node.getDrawCoordinates().x * scale),
						(int) (node.getDrawCoordinates().y * scale));
				Dimension viewableCurrentNodeBoundsScaled = new Dimension((int) (nodeTileSize * scale),
						(int) (nodeTileSize * scale));
				...
				vp.scrollRectToVisible(
						new Rectangle(viewableCurrentNodeDrawCoordinatesScaled, viewableCurrentNodeBoundsScaled));
		...
		public Dimension getPreferredSize() {
			if (graphicalSearchTree != null) {
				return new Dimension((int) ((graphicalSearchTree.getMaxDrawWidth() + 100) * scale),
						(int) ((graphicalSearchTree.getMaxDrawHeight() + 100) * scale));
			} else {
