...
import static com.jpacman.view.edumode.AIAlgorithmsStepwiseLearningRenderer.*;
...
import com.jpacman.view.edumode.AIAlgorithmsStepwiseLearningRenderer;
...
public class AIAlgorithmsStepwiseLearningController extends AIAlgorithmsLearningController implements Runnable {
	...
	public AIAlgorithmsStepwiseLearningController(TextRenderer textRenderer) {
		super(textRenderer);
		aiAlgorithmsStepwiseLearningRenderer = new AIAlgorithmsStepwiseLearningRenderer(textRenderer, pacman,
				searchAlgorithms);
		...
		if (!searchAlgorithms[algorithmIndex].isStarted() && startResumeExecuteAlgorithmButton.isClicked()) {
			...
			if (stepwiseExecutionMode) {
				if (algorithmThread == null) {
					...
					searchAlgorithms[algorithmIndex].findPath(routeTiles, ghosts[0].getCurrentTile(),
				...
				if (ghostReachedNextTile) {
					...
					List<Point> path = searchAlgorithms[algorithmIndex].findPath(routeTiles, ghosts[0].getCurrentTile(),
							pacman.getCurrentTile());
	...
	private void checkIfButtonIsClicked() {
		if (MenuController.isButtonClicked(startResumeExecuteAlgorithmButton)) {
			if (startResumeExecuteAlgorithmButton.getText().getTextMessage().equals(startText.getTextMessage())) {
			...
			} else if (startResumeExecuteAlgorithmButton.getText().getTextMessage()
					.equals(executeAlgorithmText.getTextMessage())) {
				if (!stepwiseExecutionMode && !pacman.getCurrentTile().equals(ghosts[0].getCurrentTile())) {
					...
					.equals(resumeText.getTextMessage())) {
				startResumeExecuteAlgorithmButton.setClicked(true);
				startResumeExecuteAlgorithmButton.setText(executeAlgorithmText);
				terminateThread();
				reset();
		...
		if (stepwiseExecutionMode) {
			if (stepwiseExecutionAlgorithm != null && !stepwiseExecutionAlgorithm.mutltipleIterations
					&& MenuController.isButtonClicked(executionMode)) {
			...
			if (stepwiseExecutionAlgorithm != null && stepwiseExecutionAlgorithm.stepCompleted
					&& step.getBounds().contains(Mouse.getCoordinates())) {
	...
	public void run() {
		switch (algorithmIndex) {
			...
			case SearchAlgorithm.BF:
				stepwiseExecutionAlgorithm = new BestFirstAlgorithm(
						((BestFirstAlgorithm) searchAlgorithms[algorithmIndex]).getHeuristicFunction());
				break;
			...
			case SearchAlgorithm.A_STAR:
				stepwiseExecutionAlgorithm = new AStarAlgorithm(
						((AStarAlgorithm) searchAlgorithms[algorithmIndex]).getHeuristicFunction());
				break;
			default:
