...
public class EducationalModeController implements Controller {
	...
	public static final double[] pacmanSpeeds = new double[] { 10.0, 20.0, 30.0, 40.0, 50.0, 60.0, 70.0, 80.0, 90.0,
			...
			if (algorithms[i].getBounds().contains(Mouse.getCoordinates())) {
				...
				if (Mouse.getButton() == Mouse.LEFT_BUTTON) {
					...
					if (!(i == SearchAlgorithm.BF || i == SearchAlgorithm.BS || i == SearchAlgorithm.A_STAR)) {
						...
						setButtonsClickedStatus(i, SearchAlgorithm.BF, SearchAlgorithm.BS, SearchAlgorithm.A_STAR,
								false, false);
					...
					setButtonsClickedStatus(i, SearchAlgorithm.BS, SearchAlgorithm.A_STAR, SearchAlgorithm.NONE, true,
							false);
				} else if (i == SearchAlgorithm.BS) {
					setButtonsClickedStatus(i, SearchAlgorithm.BF, SearchAlgorithm.A_STAR, SearchAlgorithm.NONE, true,
							false);
				} else if (i == SearchAlgorithm.A_STAR) {
					setButtonsClickedStatus(i, SearchAlgorithm.BF, SearchAlgorithm.BS, SearchAlgorithm.NONE, true,
							false);
				}
			} else {
	...
	private void setButtonsClickedStatus(int activeAlgorithmIndex, int inactiveAlgorithmIndex1,
			int inactiveAlgorithmIndex2, //
	...
	private void checkHeuristicButton(UIButton heuristicMenuItem, String textMessage, int i, int SearchAlgorithmIndex) {
		if (MenuController.isButtonClicked(heuristicMenuItem)) {
			...
			SearchAlgorithm.startNodeDrawable = new Node(pacman.getCurrentTile(), null);
			GraphicalSearchTreeController.graphicalSearchTree = new GraphicalSearchTree();
			EducationalModeRenderer.selectedAlgorithmText
					.setTextMessage(textMessage + heuristicMenuItem.getText().getTextMessage());
	...
	protected void checkIfIDSAlgorithmParametersAreClicked(EducationalModeRenderer renderer) {
		if (!searchAlgorithms[algorithmIndex].isRunning()
				&& MenuController.isButtonClicked(renderer.visualizeIterations)) {
		...
		if (!searchAlgorithms[algorithmIndex].isRunning()
				&& MenuController.isButtonClicked(renderer.decreaseInitialDepth)) {
			int currentInitialDepth = ((IterativeDeepeningAlgorithm) searchAlgorithms[algorithmIndex])
					.getInitialDepth();
			if (currentInitialDepth > 0) {
				((IterativeDeepeningAlgorithm) searchAlgorithms[algorithmIndex])
						.setInitialDepth(currentInitialDepth - 1);
			}
		}

		if (!searchAlgorithms[algorithmIndex].isRunning()
				&& MenuController.isButtonClicked(renderer.increaseInitialDepth)) {
			int currentInitialDepth = ((IterativeDeepeningAlgorithm) searchAlgorithms[algorithmIndex])
					.getInitialDepth();
	...
	private void transferAlgorithmRunningStatus(SearchAlgorithm currentSearchAlgorithm,
