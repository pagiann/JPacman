...
public class PacmanCollisionController extends PacmanController {
	...
	public void update(double delta) {
			...
			if (currentMode == Ghost.Mode.SCATTER || currentMode == Ghost.Mode.CHASE) {
				...
				switch (pacman.getGhostsEaten()) {
					case 1:
						...
						bonusPointsController.getBonusPointsRenderers()
								.add(new BonusPointsRenderer(new TextRenderer(new Point(0, 0)),
										ghosts[collidedGhostIndex].getPosition(),
										BonusPointsController.ONE_GHOST_EATEN_BONUS_POINTS, -xOffset, -yOffset));
					...
					case 2:
						...
						bonusPointsController.getBonusPointsRenderers()
								.add(new BonusPointsRenderer(new TextRenderer(new Point(0, 0)),
										ghosts[collidedGhostIndex].getPosition(),
										BonusPointsController.TWO_GHOSTS_EATEN_BONUS_POINTS, -xOffset, -yOffset));
					...
					case 3:
						...
						bonusPointsController.getBonusPointsRenderers()
								.add(new BonusPointsRenderer(new TextRenderer(new Point(0, 0)),
										ghosts[collidedGhostIndex].getPosition(),
										BonusPointsController.THREE_GHOSTS_EATEN_BONUS_POINTS, -xOffset, -yOffset));
					...
					case 4:
						...
						bonusPointsController.getBonusPointsRenderers()
								.add(new BonusPointsRenderer(new TextRenderer(new Point(0, 0)),
										ghosts[collidedGhostIndex].getPosition(),
										BonusPointsController.FOUR_GHOSTS_EATEN_BONUS_POINTS, -xOffset, -yOffset));
		...
		if (fruitController.getFruit().getAppeared() && pacmanCollidedWithFruit()) {
			...
			bonusPointsController.getBonusPointsRenderers()
					.add(new BonusPointsRenderer(new TextRenderer(new Point(0, 0)),
							fruitController.getFruit().getPosition(),
							fruitController.getFruit().getType().getBonusPoints(), offset / 4, offset));
			// update score
		...
		if (!pacman.getAtePill()) {
			if (pacman.getCollidedWithPill()) {
				if (pacman.isInPowerMode()) {
					// System.out.println("\npacman collided with pill and ghosts are frightened,
					// set speed
					// accordingly");
					pacman.setMovingSpeedViaMultiplier(
							levelController.getLevel().getPacmanGhostsFrightenedAndPillEatingMovingSpeedMultiplier());
					// System.out.println("changing pacman speed to " + pacman.getMovingSpeed());
				} else {
					// System.out.println("\npacman collided with pill, set speed accordingly");
					pacman.setMovingSpeedViaMultiplier(
							levelController.getLevel().getPacmanPillEatingMovingSpeedMultiplier());
					// System.out.println("changing pacman speed to " + pacman.getMovingSpeed());
				}
			...
			if (pacman.getDistanceMoved() >= Pill.SIZE / 2) { // if pacman reached the center point of pill
				...
				if (pacman.isInPowerMode()) {
					pacman.setMovingSpeedViaMultiplier(
							levelController.getLevel().getPacmanGhostsFrightenedMovingSpeedMultiplier());
				} else {
					pacman.setMovingSpeedViaMultiplier(
							levelController.getLevel().getPacmanNormalMovingSpeedMultiplier());
				}
				// System.out.println("changing pacman speed to " + pacman.getMovingSpeed());
		...
		if (!pacman.getAtePowerPill()) {
			if (pacman.getCollidedWithPill()) {
				pacman.setMovingSpeedViaMultiplier(
						levelController.getLevel().getPacmanPowerPillEatingMovingSpeedMultiplier());
				pacman.setCollidedWithPill(false);
			}
	...
	private boolean pacmanCollidedWithFruit() {
		...
		if (!pacmanCollidedWithFruitFlag) { // used to run following code only once
			...
			if (currentFruit.getAppeared() && currentFruit.getBounds().contains(pacman.getPosition())) {
				// System.out.println("\n\nPacman COLLIDED with " +
				// fruitController.getFruit().getType());
				collision = true;
