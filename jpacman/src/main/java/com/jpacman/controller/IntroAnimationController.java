...
public class IntroAnimationController implements Controller {
	...
	private Point ghostNameTextLocation = new Point(startPosition.x + Maze.WIDTH / 3 - Sprite.HALF_SPRITE_SIZE,
			startPosition.y - (Sprite.QUARTER_SPRITE_SIZE + 6));
	...
	public IntroAnimationController() {
		...
		leftEntranceVoidTileRenderer = new TileRenderer(TileRenderer.blackVoidTile, startPosition,
				Sprite.HALF_SPRITE_SIZE, Sprite.SPRITE_SIZE);
		rightEntranceVoidTileRenderer = new TileRenderer(TileRenderer.blackVoidTile, endPosition,
				Sprite.HALF_SPRITE_SIZE, Sprite.SPRITE_SIZE);
	...
	public void update(double delta) {
		...
		if (pacman.getDelta() >= 1) { // ups capped at object's current moving speed
			...
			powerPill.setAnimationCounter(
					(powerPill.getAnimationCounter() < 10000) ? powerPill.getAnimationCounter() + 1 : 0);
		}
		...
		for (int i = 0; i < ghosts.length; i++) {
			...
			if (phase == 3) {
				if (pacman.getPosition().equals(ghost.getPosition())) {
					...
					BonusPointsRenderer newRenderer = new BonusPointsRenderer(new TextRenderer(new Point(0, 0)),
							ghost.getPosition(), eatenGhostBonusPoints, -Sprite.HALF_SPRITE_SIZE,
							-Sprite.QUARTER_SPRITE_SIZE);
	...
	public void renderAll(Screen screen) {
		...
		for (GhostController ghostsController : ghostsControllers) {
			...
			} else if (ghostsController.getGhost().getPosition().x > startPosition.x
					&& ghostsController.getGhost().getPosition().x < endPosition.x) {
		...
		if (ghostNameText != null) {
			ghostNameTextRenderer.renderText(screen, ghostNameText, ghostNameTextLocation.x, ghostNameTextLocation.y,
					true);
		}
	}
}
