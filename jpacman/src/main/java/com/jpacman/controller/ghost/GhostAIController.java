...
public class GhostAIController extends GhostController
{
	 ...
	 * ghost.setMovingSpeedViaMultiplier(Ghost.GHOST_INSIDE_HOUSE_MOVING_SPEED_MULTIPLIER); } else if
	 * (!ghost.isInsideHouse() && ghost.getMovingSpeedMultiplier() ==
	 * Ghost.GHOST_INSIDE_HOUSE_MOVING_SPEED_MULTIPLIER) { if (ghost.getBeforeExitedHouseSpeedMultiplier() != 0.0) {
	...
	// ghost stays inside the house until the number of pills eaten by pacman reaches the individual ghost's limit
	// so it gets out
	if (ghost.isInsideHouse()) {
		...
		if (ghost instanceof Blinky) {
			...
			if ((ghost instanceof Pinky && pillsEatenByPacmanGlobal == 7) || (ghost instanceof Inky && pillsEatenByPacmanGlobal == 17)) {
			    ghost.setMustExitTheHouse(true);
			    ghost.setTargetTile(currentMaze.getGhostsHouseBehindDoorTile());
			    ghost.setMustExitTheHouseFlag(true);
		    ...
		    if (secElapsedSincePacmanAtePillExceedsLimit && ghost.getHouseExitPreferenceNumber() == preferredGhostToExitHouse.getValue()) {
			ghost.setMustExitTheHouse(true);
			ghost.setTargetTile(currentMaze.getGhostsHouseBehindDoorTile());
			ghost.setMustExitTheHouseFlag(true);
	    ...
	    if (!ghost.getMustEnterTheHouseFlag() && (ghost.getMode() == Ghost.Mode.EATEN) && ghost.getPosition().equals(currentMaze.getGhostsHouseInFrontOfDoorTile())) {
	...
	} else if (ghost instanceof Inky && (ghost.getHouseExitPreferenceNumber() < preferredGhostToExitHouse.getValue())) {
	    preferredGhostToExitHouse = PreferredGhost.INKY;
	} else if (ghost instanceof Clyde && (ghost.getHouseExitPreferenceNumber() < preferredGhostToExitHouse.getValue())) {
	...
	if (!(ghost instanceof Pinky) && PathFinder.isGhostHouseIntersectionTile(ghost.getPosition())) {
	    ...
	    ghost.setDirection(PathFinder.reachTargetTile(ghost.getPosition(), ghost.getTargetTile(), ghost.getDirection()));
	...
	if (ghost.getPosition().equals(currentMaze.getGhostsHouseInFrontOfDoorTile())) {
	    ...
	    ghost.setNextDirection(PathFinder.reachTargetTile(ghost.getNextTile(), ghost.getTargetTile(), ghost.getDirection()));
	...
	// only for Inky and Clyde because Blinky and Pinky only moves up-and-down inside house
	if ((ghost instanceof Inky || ghost instanceof Clyde) && PathFinder.isGhostHouseIntersectionTile(ghost.getPosition())) {
	    ghost.setDirection(PathFinder.reachTargetTile(ghost.getPosition(), ghost.getTargetTile(), ghost.getDirection()));
	}
	...
	if (ghost.getPosition().equals(ghost.getNextTile())) {
	    ...
	    if (!PathFinder.isNextTileInGhostHouseRoute(nextOfNextTile) || (!mustGetOutOfTheHouse && nextOfNextTile.equals(currentMaze.getGhostsHouseBehindDoorTile()))) {
	...
	switch (ghost.getMode()) {
	    case SCATTER:
		ghost.setTargetTile(ghost.getScatterModeTargetTile());
		if (ghost instanceof Blinky && (((Blinky) ghost).isCruiseElroy1() || ((Blinky) ghost).isCruiseElroy2())) {
	    ...
	    case FRIGHTENED:
		if (PathFinder.isIntersectionTile(ghost.getNextTile())) {
		    ghost.setNextDirection(PathFinder.getNewValidRandomDirection(ghost.getNextTile(), ghost.getDirection()));
		}
		break;
	    case EATEN:
	...
	if (ghost.getPosition().equals(ghost.getNextTile())) {
	    if (ghost.getReverseDirection() && !(ghost.getPosition().equals(currentMaze.getLeftTunnelEntranceTile()) || ghost.getPosition().equals(currentMaze.getRightTunnelEntranceTile()))) {
