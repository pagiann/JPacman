...
import jpacman.controller.edumode.AIAlgorithmsLearningController;
import jpacman.controller.edumode.AIAlgorithmsStepwiseLearningController;
import jpacman.controller.edumode.GraphicalSearchTreeController;
...
public class IterativeDeepeningAlgorithm extends SearchAlgorithm
{
	...
	while (depthLimit < Integer.MAX_VALUE && !canceled) {
		...
		if (visualizeIterations && (Game.getActiveSubMode() == SubMode.CUSTOM_SPACE_PATHFINDING || Game.getActiveSubMode() == SubMode.MAZE_PATHFINDING)) {
    ...
    {
	stepCompleted = false;
	GraphicalSearchTreeController.graphicalSearchTree = GraphicalSearchTreeController.paintableNodesOfAllTreesOfIDS.remove(0);
	...
	stepwiseExecutionCurrentActionPerforming("=> Iteration #" + iteration + ".\nSearching in depth " + currentDepth, waitingTimeInMilliseconds);
	if (!mutltipleIterations) {
	    stepwiseExecutionCurrentActionPerforming("Adding start node " + startNode.getMazeGridLocation() + "\nto the search frontier", waitingTimeInMilliseconds);
	}
	nodesVisited = 1;
	openListSize = openList.size();
	...
	AIAlgorithmsLearningController.graphicalSearchTreeController.getContentPane().getComponent(1).repaint();
	stepwiseExecutionCurrentActionPerforming("Current node's depth " + currentNode.getDepth() + ", is\nsmaller than the current\ndepth limit of " + depthLimit, waitingTimeInMilliseconds);
	stepCompleted = true;
    }
	...
	stepCompleted = false;
	waitingTimeInMilliseconds = manualExecution ? WAIT_MILLISECONDS_1 / 2 : WAIT_MILLISECONDS_2;
	stepwiseExecutionCurrentActionPerforming("Current node's depth is\nequal to the depth limit\nof " + depthLimit + ", continuing", waitingTimeInMilliseconds);
	stepCompleted = true;
    }
