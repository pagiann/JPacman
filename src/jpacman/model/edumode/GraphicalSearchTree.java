...
public class GraphicalSearchTree
{
	...
	// Initial phase of the algorithm (draw coordinates computation of nodes in last two levels)
	...
	// reverse paintableNodes list if the nodes must be painted from to top to bottom
	...
	Point coordinates;
	// System.out.println("currentMaxXDrawCoordinate = " + currentMaxXDrawCoordinate);
	...
	switch (numberOfChildNodes) {
	    case 0:
		coordinates = new Point(currentMaxXDrawCoordinate, currentMinYDrawCoordinate - yGapBetweenParentAndChildren);
		...
		coordinates = new Point(firstChildNodeDrawCoordinates.x, firstChildNodeDrawCoordinates.y - yGapBetweenParentAndChildren);
		// System.out.println("coordinates = " + coordinates);
		// System.out.println("parentNode = " + parentNode.getId());
		parentNode.drawCoordinates = coordinates;
		paintableNodes.add(parentNode);
		...
		currentMaxXDrawCoordinate = coordinates.x + nodeTileSize;
		// System.out.println("1 - currentMaxXDrawCoordinate = " + currentMaxXDrawCoordinate);
		break;
	    case 2:
		firstChildNode = childNodes.get(0); // get the 1st (child) node
		...
		int xDrawCoordForParentWithTwoChildren = firstChildNodeDrawCoordinates.x + ((secondChildNodeDrawCoordinates.x + nodeTileSize - firstChildNodeDrawCoordinates.x) / 2) - (nodeTileSize / 2);
		...
		coordinates = new Point(xDrawCoordForParentWithTwoChildren, firstChildNodeDrawCoordinates.y - yGapBetweenParentAndChildren);
		// System.out.println("coordinates = " + coordinates);
		// System.out.println("parentNode = " + parentNode.getId());
		parentNode.drawCoordinates = coordinates;
		paintableNodes.add(parentNode);
		if (parentNode.getDepth() < height - 1) {
		    checkIfSubtreeRelocationIsRequired(parentNode);
		    // currentMaxXDrawCoordinate = coordinates.x + nodeTileSize;
		    // System.out.println("4 - currentMaxXDrawCoordinate = " + currentMaxXDrawCoordinate);
		}
		if (parentNode.getDepth() == height - 1) {
		...
		int xDrawCoordForParentWithThreeChildren = firstChildNodeDrawCoordinates.x + ((thirdChildNodeDrawCoordinates.x + nodeTileSize - firstChildNodeDrawCoordinates.x) / 2) - (nodeTileSize / 2);
		int yCoordinatesOfParent = secondChildNodeDrawCoordinates.y - yGapBetweenParentAndChildren + nodeTileSize;
		...
		coordinates = new Point(xDrawCoordForParentWithThreeChildren, secondChildNodeDrawCoordinates.y - yGapBetweenParentAndChildren);
		// System.out.println("coordinates = " + coordinates);
		// System.out.println("parentNode = " + parentNode.getId());
		parentNode.drawCoordinates = coordinates;
		paintableNodes.add(parentNode);
		...
		int xDrawCoordForParentWithFourChildren = firstChildNodeDrawCoordinates.x + ((fourthChildNodeDrawCoordinates.x + nodeTileSize - firstChildNodeDrawCoordinates.x) / 2) - (nodeTileSize / 2);
		int yCoordinatesOfParent2 = secondChildNodeDrawCoordinates.y - yGapBetweenParentAndChildren + nodeTileSize;
		...
		coordinates = new Point(xDrawCoordForParentWithFourChildren, secondChildNodeDrawCoordinates.y - yGapBetweenParentAndChildren);
		parentNode.drawCoordinates = coordinates;
		paintableNodes.add(parentNode);
		if (parentNode.getDepth() < height - 1) {
		    checkIfSubtreeRelocationIsRequired(parentNode);
		...
		break;
	    default:
		System.err.println("Error: node has an invalid number of children! (" + parentNode.getChildren().size() + ")");
	}
    }
	...
	// System.out.println("2 - currentMaxXDrawCoordinate = " + currentMaxXDrawCoordinate);
	...
	if (difference < xGapBetweenSubtrees) {
	    // System.out.println("coordinates.x - (currentMaxXDrawCoordinate - xGapBetweenSubtrees) = " +
    ...
    {
	// System.out.println("node: " + node.getId() + ", node's depth:" + node.getDepth());
