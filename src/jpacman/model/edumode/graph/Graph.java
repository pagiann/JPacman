package jpacman.model.edumode.graph;
...
public class Graph
{
    ...
    // third constructor, creates a new graph whose vertices and edges are taken from the parameters.
    ...
    // adds the specified vertex to the graph, connecting it with all existing vertices and optionaly sorts the new
    // edges of the graph
	...
	if (!vertices.contains(vertexToBeAdded)) {
		...
		Edge edge = new Edge(vertexToBeAdded, vertex, Utility.computeEuclideanDistance(vertexToBeAdded, vertex));
		edges.add(edge);
	    }
    ...
    // removes the specified vertex from the graph along with all the edges that were connected to it and optionally
	...
	for (Edge edge : edges) {
		...
		System.out.println("vertex [" + vertexA.x + "," + vertexA.y + "] connects to vertex [" + vertexB.x + "," + vertexB.y + "] with weight of " + edge.getWeight() / Maze.TILE + " tile(s)");
	    }
	}
