...
public abstract class SearchAlgorithm
{
	...
	if (!stepByStepExecution) {
	    ...
	    if (Game.getActiveSubMode() == SubMode.MAZE_PATHFINDING || Game.getActiveSubMode() == SubMode.AI_ALGORITHMS_STEPWISE_LEARNING) {
	...
	if (stepByStepExecution || mutltipleIterations) {
	    stepwiseExecutionCurrentActionPerforming("Iteration #" + (++iteration) + " of algorithm", waitingTimeInMilliseconds / 4);
	}
	...
	if (nodesExpanded - numberOfExpandedNodes == 0 && openList.size() > 1) {
	    ...
	    pause();
	    unexpandedNodesDrawable.clear();
	    numberOfExpandedNodes += MIN_NUMBER_OF_EXPANDED_NODES + AIAlgorithmsInteractiveLearningController.randomNumberOfNodesToBeExpanded.nextInt(MAX_NUMBER_OF_EXPANDED_NODES);
	}
    }
