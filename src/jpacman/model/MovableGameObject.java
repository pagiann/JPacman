...
public abstract class MovableGameObject extends GameObject
{
    ...
    // ************************** Constants ************************** //
    // movable objects(pacman, ghosts, fruit?) default speed (100%) in ups (updates per second)
    ...
    // ************************* instance variables ************************ //
    // this variable determines the ups for each movable object (may be different for each object)
    ...
    // the percentage (%) with which MAX_MOVING_SPEED is multiplied to set movingSpeed
    protected double movingSpeedMultiplier = 0;
    // the current speed of all (movable) objects (this variable is changing for each object according to the current
    ...
    // the "invisible" outer tunnels that connect the left side of the maze with the right one
    protected boolean insideOuterTunnel = false;

    // move to the requested direction by one pixel, currently only orthogonal movement is supported
    ...
    {
	return (currentTile != null) ? ("[" + new Integer(currentTile.x).toString() + "," + new Integer(currentTile.y).toString() + "]") : null;
    }
    ...
    {
	return (nextTile != null) ? ("[" + new Integer(nextTile.x).toString() + "," + new Integer(nextTile.y).toString() + "]") : null;
    }
