...
import jpacman.model.ghost.Ghost.Mode;
...
public class GhostStatusController extends GhostController
{
	...
	for (int i = 0; i < numberOfTunnels; i++) {
	    ...
	    if (((currentPosition.equals(leftTunnelEntrance) && currentDirection == Direction.LEFT) || (currentPosition.equals(rightTunnelEntrance) && currentDirection == Direction.RIGHT)) && !ghost.isInsideInnerTunnel()) {
	    ...
	    if (((currentPosition.equals(leftTunnelEntrance) && currentDirection == Direction.RIGHT) || (currentPosition.equals(rightTunnelEntrance) && currentDirection == Direction.LEFT)) && ghost.isInsideInnerTunnel()) {
	...
	if (hasCurrentPeriodEnded()) {
	    // System.out.println("DEBUGGING: " + ghost.getName() + " just got to Scatter Mode");
	    scatterPeriod++;
    ...
    {
	if (hasCurrentPeriodEnded()) {
	    // System.out.println("DEBUGGING: " + ghost.getName() + " just got to Chase Mode");
	    chasePeriod++;
	...
	if (!frightenedModeInitFlag) {
	    // System.out.println("DEBUGGING: " + ghost.getName() + " just got to Frightened Mode");
	    // System.out.println("DEBUGGING... in initialization block");
	    // System.out.println("time passed = " + (oneHundredMilliseconds * 100) + " ms");
		...
		ghost.setBeforeTunnelMovingSpeedMultiplier(levelController.getLevel().getGhostFrightenedMovingSpeedMultiplier());
	...
	if (!frightenedModeFlashFlag && hasFrightenedModeFlashPeriodStarted()) {
	    // System.out.println("DEBUGGING: " + ghost.getName() + " just started flashing");
	    // System.out.println("time passed = " + (oneHundredMilliseconds * 100) + " ms");
	    ghost.setFlash(true);
	...
	if (!frightenedModeEndFlag && hasCurrentPeriodEnded()) {
	    // System.out.println("DEBUGGING: " + ghost.getName() + " getting out of Frightened mode!");
	    // System.out.println("time passed = " + (oneHundredMilliseconds * 100) + " ms");
	...
	if (ghost.getMode() == Mode.FRIGHTENED || (scatterPeriod < NUMBER_OF_SCATTER_PERIODS && chasePeriod < NUMBER_OF_CHASE_PERIODS)) {
    ...
    public static void setModePeriodsDurations(int[] scatterPeriodsDurations, int[] chasePeriodsDurations, int frightenedDuration, int numberOfFlashes)
    {
	System.arraycopy(scatterPeriodsDurations, 0, GhostStatusController.scatterPeriodsDurations, 0, scatterPeriodsDurations.length);
	System.arraycopy(chasePeriodsDurations, 0, GhostStatusController.chasePeriodsDurations, 0, chasePeriodsDurations.length);
