...
import jpacman.controller.ghost.GhostAIController.PreferredGhost;
import jpacman.controller.ghost.GhostStatusController;
import jpacman.controller.pacman.PacmanCollisionController;
import jpacman.controller.pacman.PacmanStatusController;
import jpacman.model.HighScoreEntry;
...
public class GameStatusController implements Controller, Runnable
{
    ...
    public GameStatusController(LevelController levelController, GhostStatusController[] ghostsStatusControllers, PacmanStatusController pacmanStatusController, FruitController fruitController, PillController pillsController, ScoreController scoreController, MazeRenderer mazeRenderer)
    {
	...
	pacmanStatusController.getPacman().setMovingSpeedViaMultiplier(levelController.getLevel().getPacmanNormalMovingSpeedMultiplier());
	...
	if (numberOfTopHighScores < ScoreController.NUMBER_OF_TOP_HIGH_SCORES || scoreController.getHighScore() >= ScoreController.topHighScores.get(numberOfTopHighScores - 1).getHighScore()) {

	    String username = (String) JOptionPane.showInputDialog(null, "Please enter your (nick)name: \n(only the first 10 characters will be kept)", "New Top High Score!", JOptionPane.PLAIN_MESSAGE, null, null, "player");
		...
		ScoreController.topHighScores.add(new HighScoreEntry(username, currentDate, scoreController.getHighScore()));
		...
		if (ScoreController.topHighScores.size() > ScoreController.NUMBER_OF_TOP_HIGH_SCORES) {
		    ScoreController.topHighScores = new ArrayList<HighScoreEntry>(ScoreController.topHighScores.subList(0, ScoreController.NUMBER_OF_TOP_HIGH_SCORES));
		}
