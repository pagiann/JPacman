...
import jpacman.view.edumode.EducationalModeRenderer;
import jpacman.view.edumode.TSPSolverRenderer;
...
public class TSPSolverController extends EducationalModeController implements Runnable
{
	...
	if (tspSolver.getSelectedVariation() == TSPSolver.NONE) {
		...
		if (Mouse.getButton() == Mouse.LEFT_BUTTON) {
			...
			if (!clickTile.equals(PathFinder.computeCurrentTile(pacman.getPosition()))) {
				...
				} else if ((tspSolver.getSelectedVariation() == TSPSolver.MAZE) && (routeTiles.contains(clickTile))) {
		...
		if ((tspSolver.getShortestPath() != null) && (Mouse.getButton() == Mouse.RIGHT_BUTTON)) {
		    if (tspSolver.getSelectedVariation() == TSPSolver.CLASSIC) {
			Mouse.setButton(-1);
			nextPill = new Point2D.Float(tspSolver.getShortestPath().get(0).x, tspSolver.getShortestPath().get(0).y);
	...
	if (tspSolver.getSelectedVariation() == TSPSolver.CLASSIC) {
		...
		Point nextRandomPowerPillPosition = new Point((1 + random.nextInt(25)) * Maze.TILE + offset, (1 + random.nextInt(28)) * Maze.TILE + offset);
		if (!nextRandomPowerPillPosition.equals(pacman.getPosition()) && !powerPillsPositions.contains(nextRandomPowerPillPosition)) {
	...
	if (tspSolver.getSelectedVariation() == TSPSolver.CLASSIC) {
	    ...
	    Point currentPacmanPosition = new Point((int) Math.rint(pacman.getPresicePosition().x), (int) Math.rint(pacman.getPresicePosition().y));
	    if (currentPacmanPosition.equals(tspSolver.getShortestPath().get(0))) {
		nextPill = new Point2D.Float(tspSolver.getShortestPath().get(1).x, tspSolver.getShortestPath().get(1).y);
	    ...
	    double newAngle = Math.atan2(nextPill.y - pacman.getPresicePosition().y, nextPill.x - pacman.getPresicePosition().x);
	    pacman.getPresicePosition().x += Math.cos(newAngle);
	    pacman.getPresicePosition().y += Math.sin(newAngle);

	    if (Math.rint(pacman.getPresicePosition().x) == pacman.getStartingPosition().x && Math.rint(pacman.getPresicePosition().y) == pacman.getStartingPosition().y) {
	...
	if (tspSolver.getSelectedVariation() != TSPSolver.NONE) {
	    ...
	    // if the hovered maze tile location info text must be above everything else render in this order
