...
public class PathfindingRenderer extends EducationalModeRenderer
{
    ...
    private static final Text modeSelectionText = new Text("Select a mode", Text.DEFAULT_FONT_LARGE, Screen.YELLOW_COLOR);
    ...
    private static final UIButton mazePathfindingButton = new UIButton(new Point(Maze.WIDTH, (Maze.HEIGHT / 2) + 15), true, mazeText, true);
    private static final UIButton customSpacePathfindingButton = new UIButton(new Point(Maze.WIDTH, Maze.HEIGHT / 2 + 65), true, customSpaceText, true);

    private static final Text toggleObstacleText = new Text("Right click and\ndrag to toggle\n   obstacles", Text.DEFAULT_FONT_MEDIUM, 0xFF9933);
    public static final UIButton clearPathButton = new UIButton(new Point(85, GameApplication.WINDOW_HEIGHT - GameApplication.Y_OFFSET - 110), false, new Text("Clear Path", Text.DEFAULT_FONT_MEDIUM), true);
    public static final UIButton clearAllButton = new UIButton(new Point(92, GameApplication.WINDOW_HEIGHT - GameApplication.Y_OFFSET - 80), false, new Text("Clear All", Text.DEFAULT_FONT_MEDIUM), true);
    ...
    public UIButton useDiagonalTiles = new UIButton(new Point(GameApplication.X_OFFSET + Maze.WIDTH + 30, GameApplication.Y_OFFSET + 55), false, allowDiagonalText, true);
	...
	if (Game.getActiveSubMode() == Game.SubMode.PATHFINDING_MENU) {
	    ...
	    textRenderer.renderText(screen, description, Screen.getMiddleAlignStartPositionInWindow(description.getWidth()), GameApplication.WINDOW_HEIGHT - 70, false);
	} else {
	    ...
	    expandedNodesSpeedValueText.setTextMessage("[" + String.valueOf(PathfindingController.getNodeExpansionSpeed()) + "]");
	    textRenderer.renderText(screen, expandedNodesSpeedValueText, Maze.WIDTH + 30 + expandedNodesSpeedText.getWidth() + 10, 130, true);
	    ...
	    textRenderer.renderText(screen, pacmanSpeedValueText, Maze.WIDTH + 30 + pacmanSpeedText.getWidth() + 10, 180, true);
	    ...
	    textRenderer.renderText(screen, algorithmInfoText, Maze.WIDTH + xOffsetRightPanel, yOffset1RightPanel - 15, true);
	    textRenderer.renderText(screen, selectedAlgorithmText, Maze.WIDTH + xOffsetRightPanel, yOffset1RightPanel + 5, true);
		...
		textRenderer.renderText(screen, depthStepText, Maze.WIDTH + 30, yOffset1RightPanel + yOffsetRightPanel + 5, true);
		depthStepValueText.setTextMessage("[" + ((IterativeDeepeningAlgorithm) searchAlgorithms[selectedAlgorithmIndex]).getDepthStep() + "]");
		textRenderer.renderText(screen, depthStepValueText, Maze.WIDTH + 30 + depthStepText.getWidth() + 10, yOffset1RightPanel + yOffsetRightPanel + 5, true);
		...
		textRenderer.renderText(screen, currentDepthText, Maze.WIDTH + 30, yOffset1RightPanel + yOffsetRightPanel + 25, true);
		...
		textRenderer.renderText(screen, beamWidthValueText, Maze.WIDTH + 30 + beamWidthText.getWidth() + 10, yOffset1RightPanel + 45, true);
	    ...
	    textRenderer.renderText(screen, nodesVisitedText, Maze.WIDTH + xOffsetRightPanel, yOffset1RightPanel + yOffsetRightPanel + 45, true);
	    nodesExpandedText.setTextMessage("Nodes (tiles) expanded: " + (searchAlgorithms[selectedAlgorithmIndex].getNodesExpanded()));
	    textRenderer.renderText(screen, nodesExpandedText, Maze.WIDTH + xOffsetRightPanel, yOffset1RightPanel + yOffsetRightPanel + 65, true);
	    solutionPathLengthInTiles.setTextMessage("Step: " + searchAlgorithms[selectedAlgorithmIndex].getPathInPoints().size() + " tiles");
	    textRenderer.renderText(screen, solutionPathLengthInTiles, Maze.WIDTH + xOffsetRightPanel, yOffset1RightPanel + yOffsetRightPanel + 85, true);
	    if (Game.getActiveSubMode() == SubMode.CUSTOM_SPACE_PATHFINDING) {
		...
		textRenderer.renderText(screen, solutionPathLengthInPixels, Maze.WIDTH + xOffsetRightPanel, yOffset1RightPanel + yOffsetRightPanel + 105, true);
		yOffset += 20;
	    }
	    ...
	    // pixels");
	    // textRenderer.renderText(screen, distanceTraveledText, Maze.WIDTH + xOffsetRightPanel, yOffset + 40,
	    // true);
